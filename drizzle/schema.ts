...
export const orderItems = mysqlTable("order_items", {
  ...
  estoqueReservado: boolean("estoqueReservado").default(false), // true = estoque reservado no Maxiprod, false = apenas "reservar" (pendente)
  collectedAt: timestamp("collectedAt").defaultNow().notNull(),
});
...
export const collectionActions = mysqlTable("collection_actions", {
  ...
  status: varchar("status", { length: 30 }).notNull().default("pendente"),
    // pendente | contatado | em_negociacao | promessa | nao_retornou | nao_atendeu | protesto_em_analise | protestado | juridico | especial_sem_cobranca | cheque_compensacao
...
export const salesOrderRequests = mysqlTable("sales_order_requests", {
  ...
  status: mysqlEnum("status", ["pendente", "aprovado", "aprovado_subgestor", "rejeitado", "processado", "simulacao"]).default("pendente").notNull(),
  ...
  tipoFaturamento: varchar("tipo_faturamento", { length: 30 }), // Tipo de Faturamento: Normal, Zap 0, Zap 2, Zap 3, Zap 5
...
export const vendorClients = mysqlTable("vendor_clients", {
  ...
  source: varchar("source", { length: 20 }).default("manual"), // 'manual' | 'maxiprod' | 'pedido'
...
export const importPayments = mysqlTable("import_payments", {
  ...
  totalUsd: decimal("total_usd", { precision: 18, scale: 10 }).notNull(),
  halfValue: decimal("half_value", { precision: 18, scale: 10 }), // DEPRECATED - mantido por compatibilidade
  totalBrasilUsd: decimal("total_brasil_usd", { precision: 18, scale: 10 }).default("0").notNull(), // Brasil na seção "Total a pagar" (azul)
  totalParaguaiUsd: decimal("total_paraguai_usd", { precision: 18, scale: 10 }).default("0").notNull(), // Paraguai na seção "Total a pagar" (azul)
  brasilUsd: decimal("brasil_usd", { precision: 18, scale: 10 }).default("0").notNull(), // Brasil na seção "O que pagou" (verde)
  paraguaiUsd: decimal("paraguai_usd", { precision: 18, scale: 10 }).default("0").notNull(), // Paraguai na seção "O que pagou" (verde)
  totalPago: decimal("total_pago", { precision: 18, scale: 10 }).default("0").notNull(),
  saldoDevedorBrasil: decimal("saldo_devedor_brasil", { precision: 18, scale: 10 }).default("0").notNull(),
  saldoDevedorParaguai: decimal("saldo_devedor_paraguai", { precision: 18, scale: 10 }).default("0").notNull(),
  saldoDevedorTotal: decimal("saldo_devedor_total", { precision: 18, scale: 10 }).default("0").notNull(),
...
export const sellerAlerts = mysqlTable("seller_alerts", {
  ...
  cancelledBy: varchar("cancelled_by", { length: 200 }),
