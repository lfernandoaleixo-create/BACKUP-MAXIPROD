...
export default function GestorAprovacoes(props: any = {}) {
  ...
  const orders = useMemo(() => {
    ...
    // Filter out orders being edited by someone else (edit lock)
    filtered = filtered.filter((o: any) => {
      if (!o.editingBy) return true; // not being edited
      // Auto-unlock after 30 min
      if (o.editingAt && Date.now() - o.editingAt > 30 * 60 * 1000) return true;
      // If the current gestor is the one editing, still show it
      if (gestorName && o.editingBy === gestorName) return true;
                ...
                >
                  ...
                  >
                    ...
                    <div className="text-right flex-shrink-0 flex items-center gap-1 sm:gap-2 flex-wrap">
                      ...
                      {(order.status === "pendente" || order.status === "aprovado_subgestor" || order.status === "aprovado" || order.status === "rejeitado") && (
                        <button
                          onClick={async (e) => { e.stopPropagation(); try { await fetch("/api/trpc/salesOrders.startEditing", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({json:{orderId: order.id, editorName: gestorName || "Gestor"}}) }); } catch(err) {} window.location.href = `/gestao-comercial/vendedor/${order.sellerId}?editOrder=${order.id}`; }}
                        ...
                        {(order.segmento || order.condicaoPagamento || order.formaCobranca || order.limiteCredito || order.tabelaPrecos || order.observacoes) && (
                          <div className="bg-green-50/50 dark:bg-green-900/10 rounded-lg p-3 border border-green-200 dark:border-green-700">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-1.5 text-[10px]">
                              ...
                              {(order as any).tipoFaturamento && (order as any).tipoFaturamento !== "Normal" && (<div><span className="text-slate-400 font-semibold">Tipo de Faturamento</span><p className="text-red-600 dark:text-red-400 font-bold">{(order as any).tipoFaturamento}</p></div>)}
                              ...
                              {(order as any).observacoesInternas && (<div className="col-span-2 md:col-span-3 bg-blue-50 dark:bg-blue-900/20 p-2 rounded"><span className="text-blue-600 font-semibold text-xs">📝 Obs. Internas (Fiscal/Financeiro)</span><p className="text-slate-800 dark:text-slate-100 whitespace-pre-wrap text-sm mt-1">{(order as any).observacoesInternas}</p></div>)}
                              {(order as any).observacoesInternas && (<div className="col-span-2 md:col-span-3 bg-blue-50 dark:bg-blue-900/20 p-2 rounded"><span className="text-blue-600 font-semibold text-xs">📝 Obs. Internas (Fiscal/Financeiro)</span><p className="text-slate-800 dark:text-slate-100 whitespace-pre-wrap text-sm mt-1">{(order as any).observacoesInternas}</p></div>)}
                            </div>
                          </div>
                        )}
