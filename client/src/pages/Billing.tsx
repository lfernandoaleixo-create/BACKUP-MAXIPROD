  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-72 p-3" align="start">
        <p className="text-xs font-semibold text-slate-700 mb-2">Histórico de Transportadora</p>
        {isLoading ? (
          <div className="flex items-center gap-2 text-xs text-slate-400">
            <Loader2 className="w-3 h-3 animate-spin" /> Carregando...
          </div>
        ) : !history || history.length === 0 ? (
          <p className="text-xs text-slate-400 italic">Nenhuma troca registrada</p>
        ) : (
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {history.map((h) => (
              <div key={h.id} className="border-l-2 border-blue-300 pl-2 py-1">
                <div className="flex items-center gap-1 text-[10px] text-slate-400">
                  <span>{new Date(h.createdAt).toLocaleDateString("pt-BR")} {new Date(h.createdAt).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</span>
                  <span>•</span>
                  <span className="font-medium text-slate-600">{h.alteradoPor}</span>
                </div>
                <div className="text-[11px] text-slate-600 mt-0.5">
                  {h.transportadoraAnterior ? (
                    <span>
                      <span className="text-red-500 line-through">{TRANSPORT_LABELS[h.transportadoraAnterior] || h.transportadoraAnterior}</span>
                      {" \u2192 "}
                      <span className="text-green-600 font-medium">{TRANSPORT_LABELS[h.transportadoraNova] || h.transportadoraNova}</span>
                    </span>
                  ) : (
                    <span>
                      Definida como <span className="text-green-600 font-medium">{TRANSPORT_LABELS[h.transportadoraNova] || h.transportadoraNova}</span>
                    </span>
                  )}
                </div>
              </div>
        ...
        {onToggleCollection && (() => {
          ...
          <div className="flex-shrink-0 flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
            ...
            {onChangeTransportadora && (
              <div className="w-36 flex-shrink-0 flex items-center justify-center gap-1">
                ...
                <TransportHistoryPopover pedido={order.pedido} />
              </div>
            )}
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex-shrink-0 text-right">
                    <span className="text-base font-bold text-slate-800">
                       {formatNumber(item.quantidade)} {item.unidadeMedida || 'un'}
                    </span>
                ...
                <div className="text-right">
                  <div className="border-t-2 border-slate-400 pt-1.5 mt-0.5 pl-8">
                    <span className="text-sm font-bold text-slate-800">
                       Total: {formatNumber(order.itens.reduce((sum, i) => sum + i.quantidade, 0))} {order.itens[0]?.unidadeMedida || 'un'}
                    </span>
                    {showValues && (
