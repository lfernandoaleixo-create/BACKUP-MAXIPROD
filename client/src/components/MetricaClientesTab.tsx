        ...
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-blue-800 dark:text-blue-300">
            ...
            <ul className="list-disc list-inside space-y-1 text-xs">
              <li><strong>Cliente Novo:</strong> Primeira compra na empresa OU não comprava há 6+ meses (reativação)</li>
              <li><strong>Cliente Herdado:</strong> Já comprava antes do vendedor entrar (comprou nos últimos 6 meses antes da admissão)</li>
              <li><strong>Cliente Reativado:</strong> Não comprava há 6+ meses antes da admissão, mas voltou a comprar com o vendedor</li>
            </ul>
            <p className="mt-2 text-xs italic">Preencha a data de admissão de cada vendedor para ativar as métricas.</p>
          </div>
        </div>
      </div>
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {SELLERS.map((seller) => (
              <div key={seller} className="px-4 py-3">
                ...
                {/* Metrics preview (if admission date exists) */}
                {hasAdmissionDate(seller) && (
                  <div className="mt-2">
                    <button
  ...
  return (
    <div className="mt-3 space-y-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg p-2.5">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-[10px] font-medium text-emerald-700 dark:text-emerald-300 uppercase">Novos</span>
          </div>
          <p className="text-lg font-bold text-emerald-800 dark:text-emerald-200">{metrics.clientesNovos}</p>
        </div>
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-2.5">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-[10px] font-medium text-amber-700 dark:text-amber-300 uppercase">Reativados</span>
          </div>
          <p className="text-lg font-bold text-amber-800 dark:text-amber-200">{metrics.clientesReativados}</p>
        </div>

        <div className="bg-slate-50 dark:bg-slate-700/40 border border-slate-200 dark:border-slate-600 rounded-lg p-2.5">
          <div className="flex items-center gap-1.5 mb-1">
            <UserCheck className="w-3.5 h-3.5 text-slate-600 dark:text-slate-400" />
            <span className="text-[10px] font-medium text-slate-700 dark:text-slate-300 uppercase">Herdados</span>
          </div>
          <p className="text-lg font-bold text-slate-800 dark:text-slate-200">{metrics.clientesHerdados}</p>
        </div>

        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-2.5">
          <div className="flex items-center gap-1.5 mb-1">
            <Users className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
            <span className="text-[10px] font-medium text-blue-700 dark:text-blue-300 uppercase">Total</span>
          </div>
          <p className="text-lg font-bold text-blue-800 dark:text-blue-200">{metrics.totalClientes}</p>
        </div>
      </div>

      {/* Client lists */}
      {metrics.clientesNovos > 0 && (
        <div className="bg-emerald-50/50 dark:bg-emerald-900/10 rounded-lg p-3 border border-emerald-100 dark:border-emerald-800/50">
          ...
          <div className="flex flex-wrap gap-1">
            {metrics.listaClientesNovos.map((c, i) => (
              <span key={i} className="text-[10px] bg-emerald-100 dark:bg-emerald-800/40 text-emerald-800 dark:text-emerald-200 px-1.5 py-0.5 rounded">
                {c}
              </span>
            ))}
      ...
      {metrics.clientesReativados > 0 && (
        <div className="bg-amber-50/50 dark:bg-amber-900/10 rounded-lg p-3 border border-amber-100 dark:border-amber-800/50">
          <h4 className="text-xs font-semibold text-amber-700 dark:text-amber-300 mb-1.5">
            Clientes Reativados ({metrics.clientesReativados})
          </h4>
          <div className="flex flex-wrap gap-1">
            {metrics.listaClientesReativados.map((c, i) => (
              <span key={i} className="text-[10px] bg-amber-100 dark:bg-amber-800/40 text-amber-800 dark:text-amber-200 px-1.5 py-0.5 rounded">
                {c}
              </span>
            ))}
