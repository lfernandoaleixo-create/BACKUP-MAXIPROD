...
`.trim();
  ...
  /**
   * getCheques - Busca todos os cheques do Contas a Receber
   * Organizado por empresa → mês (vencimento) → estado do cheque
   * Filtrável por: empresa, mês, estado do cheque
   */
  getCheques: publicProcedure
    .input(z.object({
      empresa: z.string().optional(), // PALITOS, VARETAS, ESPETOS ou undefined (todas)
      mes: z.string().optional(), // YYYY-MM ou undefined (todos)
      chequeEstado: z.string().optional(), // Um dos 9 estados ou undefined (todos)
    }).optional())
    .query(async ({ input }) => {
      const db = await getDb();
      if (!db) return { empresas: [], totals: { total: 0, count: 0 } };

      // Buscar todos os cheques (formaCobranca começa com "Cheque")
      const conditions = [
        inArray(accountsReceivable.tipo, RECEIVABLE_VALID_TYPES),
        sql`UPPER(${accountsReceivable.formaCobranca}) LIKE 'CHEQUE%'`,
        eq(accountsReceivable.estado, "EMITIDO"),
      ];
      ...
      // Extract cheque state from formaCobranca
      // Pattern: "Cheque CHEQUE <STATE> ag - conta -" or "Cheque CHEQUE <STATE>"
      function extractChequeState(forma: string): string {
        if (!forma) return "OUTROS";
        // Remove prefix "Cheque " and extract the state part
        const upper = forma.toUpperCase();
        if (upper.includes("CHEQUE DISPONIVEL") || upper.includes("CHEQUE DISPONÍVEL")) return "CHEQUE DISPONÍVEL";
        if (upper.includes("CHEQUE À RECEBER DE CLIENTES") || upper.includes("CHEQUE A RECEBER DE CLIENTES")) return "CHEQUE À RECEBER DE CLIENTES";
        if (upper.includes("CHEQUE EM COMPENSACAO") || upper.includes("CHEQUE EM COMPENSAÇÃO")) return "CHEQUE EM COMPENSAÇÃO";
        if (upper.includes("CHEQUE CUSTODIA SICOOB") || upper.includes("CHEQUE CUSTÓDIA SICOOB")) return "CHEQUE CUSTÓDIA SICOOB";
        if (upper.includes("CHEQUE CUSTODIA SICREDI") || upper.includes("CHEQUE CUSTÓDIA SICREDI")) return "CHEQUE CUSTÓDIA SICREDI";
        if (upper.includes("CHEQUE LINHA 11")) return "CHEQUE LINHA 11";
        if (upper.includes("CHEQUE LINHA 12")) return "CHEQUE LINHA 12";
        if (upper.includes("CHEQUE VOLTOU OUTROS MOTIVOS")) return "CHEQUE VOLTOU OUTROS MOTIVOS";
        if (upper.includes("CHEQUE EM FACTORING")) return "CHEQUE EM FACTORING";
        return "OUTROS";
      }

      // Simplify empresa name
      function shortEmpresa(nome: string): string {
        const u = (nome || "").toUpperCase();
        if (u.includes("PALITOS")) return "PALITOS";
        if (u.includes("VARETAS")) return "VARETAS";
        if (u.includes("ESPETOS")) return "ESPETOS";
        if (u.includes("MESA")) return "MESA";
        return nome || "Sem Empresa";
      }
      ...
      for (const row of rows) {
        ...
        const empresaShort = shortEmpresa(row.empresaNome || "");
        const chequeEstado = extractChequeState(row.formaCobranca || "");
        const vencDate = (row.vencimentoData || "").split("T")[0];
        const mesKey = vencDate.substring(0, 7);

        // Apply filters
        if (input?.empresa && empresaShort !== input.empresa) continue;
        if (input?.mes && mesKey !== input.mes) continue;
        if (input?.chequeEstado && chequeEstado !== input.chequeEstado) continue;
        ...
        items.push({
          ...
          vencimento: vencDate,
          emissao: (row.emissaoData || "").split("T")[0],
          empresa: row.empresaNome || "Sem Empresa",
          empresaShort,
          chequeEstado,
          formaCobranca: row.formaCobranca || "",
      ...
      // Group by empresa → mês
      type MesGroup = { mes: string; items: ChequeItem[]; total: number; count: number };
      type EmpresaGroup = { nome: string; meses: MesGroup[]; total: number; count: number };

      const empresaMap: Record<string, { meses: Record<string, ChequeItem[]>; total: number; count: number }> = {};
      for (const item of items) {
        const emp = item.empresaShort;
        const mes = item.vencimento.substring(0, 7);
        if (!empresaMap[emp]) empresaMap[emp] = { meses: {}, total: 0, count: 0 };
        if (!empresaMap[emp].meses[mes]) empresaMap[emp].meses[mes] = [];
        empresaMap[emp].meses[mes].push(item);
        empresaMap[emp].total += item.valor;
        empresaMap[emp].count++;
      }

      // Convert to sorted arrays
      const empresas: EmpresaGroup[] = Object.entries(empresaMap)
        .sort(([a], [b]) => a.localeCompare(b))
        .map(([nome, data]) => ({
          nome,
          total: data.total,
          count: data.count,
          meses: Object.entries(data.meses)
            .sort(([a], [b]) => a.localeCompare(b))
            .map(([mes, items]) => ({
              mes,
              items: items.sort((a, b) => a.vencimento.localeCompare(b.vencimento)),
              total: items.reduce((s, i) => s + i.valor, 0),
              count: items.length,
            })),
        }));

      // Summary by cheque state
      const estadoSummary: Record<string, { count: number; total: number }> = {};
      for (const item of items) {
        if (!estadoSummary[item.chequeEstado]) estadoSummary[item.chequeEstado] = { count: 0, total: 0 };
      ...
      return {
        empresas,
        totals: { total: items.reduce((s, i) => s + i.valor, 0), count: items.length },
        estadoSummary,
        availableEmpresas: Array.from(new Set(items.map(i => i.empresaShort))).sort(),
        availableMeses: Array.from(new Set(items.map(i => i.vencimento.substring(0, 7)))).sort(),
      };
    }),
});